...
ConnectionResult=`mysql --defaults-extra-file=${file} -e "show slave ${connection} status\G" 2>&1`
if [ -z "`echo "${ConnectionResult}" |grep Slave_IO_State`" ]; then
	echo -e ${ConnectionResult}
	exit ${STATE_CRITICAL}
fi
